...
- [x] Creare backup in cartella esterna all'app (download?)
...
- [ ] CmkWeb: sourceId meglio che derivi dal comics stesso (hash del nome?) e non creato random (uuid) perché ricreando il DB verrebbero persi i collegamenti con i dati salvati nell'app

## Note

### Rivista funzionalità per _linkare_ i comics con CmkWeb

Il selettore (`ComicsSelectorFragment`) non mi convince e per ora ho nascosto il punto di menu.
Soprattutto il caricamento dei dati da web è molto lento.

Il _link_ lo eseguo a livello di singolo comics da apposita voce di menu (`Follow`).
